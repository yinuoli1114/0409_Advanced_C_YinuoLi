...
SparseNode * SparseArray_copy(SparseNode * array)
{
  ...
  if(array != NULL)
  {
    copy = SparseNode_create(array -> index, array -> value);
 ...
 */
SparseNode * mergehelper(SparseNode * new_array, SparseNode * array_2);
SparseNode * decide(SparseNode * new_array, SparseNode * node2);

SparseNode * SparseArray_merge(SparseNode * array_1, SparseNode * array_2)
{
  SparseNode * new_array = NULL;
  new_array = SparseArray_copy(array_1);
  return new_array = mergehelper(new_array, array_2);
}
...
{ 
  ...
  new_array = mergehelper(new_array, array_2 -> left);
  new_array = mergehelper(new_array, array_2 -> right);
  new_array =  decide(new_array, array_2);
  return new_array;
}

SparseNode * decide(SparseNode * new_array, SparseNode *  node2)
{
  if(SparseArray_getNode(new_array, node2 -> index) == NULL)
  {
    new_array = SparseArray_insert(new_array, node2 -> index, node2 -> value);
  }
  else
  {
    SparseArray_getNode(new_array, node2 -> index) -> value = SparseArray_getNode(new_array, node2 -> index) -> value + node2 -> value;
    if(SparseArray_getNode(new_array, node2 -> index) -> value == 0)
    {
      new_array = SparseArray_remove(new_array, node2 -> index);
    }
  }
  return new_array;
}
