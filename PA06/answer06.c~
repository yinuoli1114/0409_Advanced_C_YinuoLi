 ...
 *     will be the same. 
 *
 * (8) Don't leav this to the last minute.
 *
 */
...
struct Image * loadImage(const char* filename)
{
  ...
  */
  ...
  //fread(&hdr1.magic_bits, sizeof(int),1,fptr);
  if(hdr1.magic_bits != ECE264_IMAGE_MAGIC_BITS)
  {
    fclose(fptr);
    return NULL;
  }
 //fread(&hdr1.width,sizeof(int) , 1, fptr); 
  if(hdr1.width <= 0)
    {
      fclose(fptr);
      return NULL;
      }
  //fread(&hdr1.height, sizeof(int), 1, fptr);
  if(hdr1.height <= 0)
    {
      fclose(fptr);
      return NULL;
  ...
  int * temp = malloc(sizeof(uint8_t) * 1);
...
/*
 * ===================================================================
 * Performs "linear normalization" on the passed imag
 *
...
void linearNormalization(struct Image * image)
{
  ...
  {
    pixel[ind] = (pixel[ind] - min) * 255.0 / (max - min);
  }
